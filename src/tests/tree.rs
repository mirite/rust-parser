...
mod tests {
    ...
    fn it_can_handle_a_simple_tree() {
        ...
        assert_eq!(tree[0].token, tokens[0]);
        assert_eq!(tree[0].children.len(), 1);
        assert_eq!(tree[0].children[0].token, tokens[1]);
    }

    #[test]
    fn it_can_handle_a_nested_tree() {
        let tokens = vec![
            types::Token {
                name: Some("div".to_string()),
                tag_type: Some(types::TagType::Open),
        ...
        assert_eq!(tree[0].children[0].token, tokens[1]);
        assert_eq!(tree[0].children[0].children[0].token, tokens[2]);
    }
}
